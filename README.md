...
Вместо того, что бы изменять состояние напрямую, вы определяете изменения, которые вы хотите произвести, с помощью простых объектов называемых _экшенами_. Затем вы пишете специальную функцию, называемую _редюсер_, чтобы решить, каким образом каждый экшен преобразует все состояние приложения.
