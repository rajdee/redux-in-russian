...
Давайте напишем функцию, которая позволит нам выражать редюсеры как объект, сопоставляющий типы экшенов обработчикам. Например, если мы хотим описывать наши редюсеры для `todos` так:
