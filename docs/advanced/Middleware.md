...
Но есть еще другой метод реализации объединения мидлваров в цепочки (chaining). мидлвар мог бы принимать функцию отправки действия `next()` в параметрах вместо того, чтобы читать ее из экземпляра хранилища.
...
Теперь мидлвар принимает функцию отправки действия `next()` и возвращает другую функцию отправки действия, которая, в свою очередь, является функцией отправки действия `next()` для мидлвара слева. Все еще полезно иметь доступ к некоторым методам хранилища, например к `getState()`, следовательно, `store` остается доступен как аргумент самого верхнего уровня.
...
* Она предоставляет мидлвару подмножество [API хранилища](../api/Store.md): методы [`dispatch(action)`](../api/Store.md#dispatch) и [`getState()`](../api/Store.md#getState).

* Она использует некоторые хитрости для того, чтобы убедиться, что, действие снова пройдет через всю цепочку мидлваров, включая текущий, если вы вызываете `store.dispatch(action)` из вашего мидлвара вместо `next(action)`. Это полезно для асинхронных мидлваров, как мы [ранее](AsyncActions.md) видели.
 ...
 * Этот мидлвар превратит их в единственное действие в начале,
